...
var Sim = (function () {
  ...
  /* 结算维持费。逻辑：
   *   ① 正常扣水；
   *   ② **只有水真的见了底（≤ 0）才开始降级** —— 只要还有余额就一直正常供养；
   *   ③ 见底后按固定速度降（downgradePerSec 级/秒），远的先降（锁定的最后）；
   *   ④ 水不许变成负数 —— 见底就归零，缺口由降级补上。
   *
   * 降级**只减等级、绝不移除节点**（守住「绝不永久损失」底线）。
   * 降级的代价不在水里，而在「产出和吞吐掉回上一级」——
   * 所以它读起来是「你维持不起了，撤回一部分投资」，而不是「系统罚你」。
   *
   * 【为什么触发条件是「水见底」，而不是「低于某条缓冲线」】
   * 旧版在水量低于「维持费 × bufferSec（40 秒）」时就开始降级。后果是
   * **水还是正数的时候等级就在往下掉** —— 面板上那行还没见底就已经红了。
   * 玩家反馈：「是我的总水分变成 0 了才开始降级」。这个直觉是对的：
   * 水就是水，还有余额就说明还养得起。缓冲线已整个去掉。
   *
   * 【为什么速度是固定值，而不是随缺水程度缩放】
   * 旧版按「离缓冲线多远」缩放（0.15 ~ 3 倍）。缓冲线一去掉，这个缩放就
   * 失去了量纲 —— 见底之后水位恒为 0，没有可缩放的量。
   * 期间试过两种替代，都否掉：
   *   · 拿「本来会扣成多负」当缺水量：那个值在见底第一帧就接近满值，
   *     等于「刚见底就按最高速砸」，玩家刚买个升级花光水，一两秒掉十几级；
   *   · 按「已连续缺水多久」爬升：见底后头一两秒配额攒不够 1 级 →
   *     一级都不掉，又和「水到 0 就开始降级」相反。
   * 固定速度两头都不占，语义也最直白：水到 0 → 每秒降 3 级。
   * 要调手感直接改 downgradePerSec，不需要再叠一层曲线。
   */
  /* 核心渗水/秒（含「渗透」基因加成）。
  ...
  function settleMaintenance(state, dt) {
    ...
    /* 触发条件就是这一句：**水见底了**。
     * 还有余额（> 0）就老老实实供养，一级都不掉 —— 这是玩家要的语义。 */
    if (state.res.water <= 0) {
      /* 见底之后按固定速度降（downgradePerSec 级/秒）。
       *
       * 【为什么不做「按缺水程度缩放」或「随时间爬升」】
       * 两者都试过、都否掉了：
       *   · 按缺水程度缩放：得先定义「缺多少」—— 见底之后水位恒为 0，
       *     没有任何可供缩放的量纲，只能拿「本来会扣成多负」来替代，
       *     而那个值在见底的第一帧就接近满值 → 等价于「刚见底就按最高速砸」，
       *     玩家只是买了个升级把钱花光，一两秒内掉十几级，来不及反应。
       *   · 随时间爬升：则见底后头一两秒**一级都不掉**（配额还没攒够 1 级），
       *     又和「水到 0 就开始降级」的预期相反。
       * 固定速度两头都不占：语义直白（水到 0 → 每秒降 3 级），
       * 也保住了「渐进、来得及救」。速度本身就是旋钮，要调直接改它。 */
      var quota = CONFIG.MAINT.downgradePerSec * dt;

      /* 配额不足 1 级时不能白扔 —— 攒起来。
       *
       * 为什么必须有这个累加器：浏览器以 60fps 跑，单帧 dt ≈ 0.0167。
       * 单帧 quota ≈ 3 * 0.0167 ≈ 0.05 级/帧，恒定小于 1 ——
       * 如果直接丢掉，降级永远不会发生（实际表现是彻底卡住不动）。
       * 累积到 1 级再一次性降，等价于「平均每秒降 downgradePerSec 级」，
       * 既保住了渐进的手感，也保住了「确实在收缩」的语义。 */
       ...
       *
       * 差别在配额 < 1 时的行为，非常关键：
       *   budget = 0（刚见底）时，若写成 `downgraded < quota`，
       *   第一次判断 0 < 0.45 成立 → 降 1 级 ——
       *   于是**任何非零配额都至少降 1 级**，降级速度在低端被量化成
       *   固定每秒 1 级，缩放完全失效，玩家看到的是「刚一缺水就猛地掉级」。
      ...
      /* 还有水：累加器也清掉，恢复供水后不会「攒着火气」立刻连降。 */
      state.maintainQuota = 0;
    }

    /* 告警的含义就是「水见底了、正在掉级」—— 面板上水分那行变红即此刻。 */
    state.maintainPressure = state.res.water <= 0;
    /* 水不许变成负数：真见底就归零。
