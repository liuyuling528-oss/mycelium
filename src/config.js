...
var CONFIG = (function () {
   ...
   *
   * 等级规则：感染只往「等级 ≤ 源头」的邻居菌丝传，且自动感染封顶 maxLevel ——
   * Lv8+ 的菌完全免疫。铺广流（大片 Lv0）天然易感，深耕流（少数高等级菌）
   * 自带防火墙，「深耕 vs 铺广」这个取舍因此多了一层攻防含义。
   *
   * **不能手动净化**：点一下就消掉还倒贴奖励，菌瘟就不配叫威胁了 ——
   * 玩家的应对只有两个：把关键菌丝练到 Lv8+ 修「防火墙」阻断去路，
   * 或者等它自愈。等级规则因此是实打实的防御工事，而不是装饰。
   *
   * 但它**会自愈**：放着不管 recoverT 秒自己好，绝不造成永久损失 ——
   * 底线不变：挑战来自「防火墙修在哪、放弃哪一片」，绝不「回来一看全毁了」。
   *
  ...
  var BLIGHT = {
    ...
    maxCount: 8,           // 同时最多几处
    maxLevel: 7,           // 自动感染的最高节点等级：Lv8+ 免疫菌瘟（深耕的回报）
  ...
  var MILESTONES = [
    ...
    { id: 'm9', name: '防火墙', need: '同时拥有 3 个 Lv8+ 的菌丝节点', reward: '菌瘟蔓延速度降低' },
