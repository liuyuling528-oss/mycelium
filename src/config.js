...
var CONFIG = (function () {
   ...
   *
   * costPerLevel 是扫出来的（tools/probe_cap.js / probe_ceil.js）。
   * 关键判据是「**可维持的等级总和**」= 水产量 ÷ k：
   * 玩家的养分预算能练出多少「级」是有限的，只要水能养住的级数
   * 明显少于他能练出的级数，他就**必须在节点之间取舍**。
   *
   * 实测（20 分钟，tools/probe_cap.js / probe_nutrient.js）：
   *   · 后期水产量 ≈ 554/s（稳态）
   *   · 玩家累计养分 ≈ 48,736 → 约能练出 143 级
   * 于是：
   *   k=0.25 → 可养 2214 级，远超 143 → **水完全不构成约束**（机制等于不存在）
   *   k=0.60 → 可养  923 级，仍远超 143 → 且降级成了「一练就掉」的噪音
   *   k=4.00 → 可养  138 级，≈ 玩家刚好能练出的量 → 边界
   *   **k=6.00 → 可养 92 级，玩家练得出 143 级却只养得起 92 级** ← 采用
   * 即：玩家必须放弃约 1/3 的等级投入，取舍成立。
   */
  var MAINT = {
    /* 每个等级每秒耗水（Lv10 节点 = 60/s）。
     * 参照：后期水产量 ≈ 554/s → 可维持「等级总和 ≈ 92」
     * （约 9 个 Lv10 节点，或 30 个 Lv3 节点）。 */
    costPerLevel: 6.0,
